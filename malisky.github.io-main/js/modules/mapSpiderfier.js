...
function getCountryFlagEmoji(country) {
  ...
  // Custom spread logic for Kazakhstan
  spiderfier.spreadMarkers = function(country) {
    ...
    const radius = 0.03; // adjust as needed
  ...
  // Reset spiderfied markers
  spiderfier.unspreadMarkers = function() {
  ...
  // Hook into marker clicks
  spiderfier.markers.forEach(marker => {
    marker._originalLatLng = marker.getLatLng();
    marker.on('click', () => {
      if (marker.countryGroup === 'Kazakhstan') {
        if (spiderfier.spiderfied) {
          spiderfier.unspreadMarkers();
        } else {
          spiderfier.spreadMarkers('Kazakhstan');
        }
      }
    });
  });
